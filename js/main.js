...
select.addEventListener("change", (event) => {
  //when a new option appears in the box, pass in that change event
...
// This is the .then fetch for API data
...
// This same function uses Async/Await to fetch API data
async function getDoggoImg(url) {
  try{
...
const doggoInfo = {
  ...
  assignMF() {
    x = Math.floor(Math.random() * 2) == 0; // Random Boolean result (T or F)
    if (x) {
      this.MF = "Female";
  ...
  assignName(array) {
    this.rname = array[Math.floor(Math.random() * array.length)]; // get a random name in the specific name array
    document.getElementById("dog-name").innerHTML = `${this.rname}`;
  },

  assignAge() {
    this.age = Math.floor(Math.random() * 10) + 1;
    document.getElementById("age").innerHTML = `${this.age} years old`;
  },
  ...
  assignLikes() {
    this.likes = this.yatesShuffle(this.likesList).slice(0, 2); // do a shuffle of the likes list, grab first two
  ...
  assignDislikes() {
    this.dislikes = this.yatesShuffle(this.dislikesList).slice(0, 2); // do a shuffle of the likes dislist, grab first two
    document.getElementById(
      "dislikes"
